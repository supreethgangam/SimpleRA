...
void executeJOIN()
{
    ...
    parsedQuery.sortStartegyList = {"ASC"} ;
    parsedQuery.sortColumnList = {parsedQuery.joinFirstColumnName} ;
    Table* res1 = table1->sort() ;
    parsedQuery.sortColumnList.clear() ;
    parsedQuery.sortColumnList = {parsedQuery.joinSecondColumnName} ;
    // parsedQuery.sortStartegyList = {"ASC"} ;
    Table* res2  = table2->sort() ;
    Table* res = res1->join(res2) ;
    // cout<<"hello\n" ;
    res->blockify()  ;
    // cout<<"bye\n" ; 
...
//  PRANAV <- JOIN Student, Advisor ON Advisor_id == Adv_Id
